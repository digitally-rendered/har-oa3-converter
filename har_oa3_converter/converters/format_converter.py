...
class HarToOpenApi3Converter(FormatConverter):
    ...
    def convert(self, source_path: str, target_path: Optional[str] = None, **options) -> Dict[str, Any]:
        ...
        return converter.convert(source_path, target_path, validate_schema=False)  # Validation already done above
...
class OpenApi3ToOpenApi3Converter(FormatConverter):
    ...
    def convert(self, source_path: str, target_path: Optional[str] = None, **options) -> Dict[str, Any]:
        ...
        if options.get("servers"):
            openapi3["servers"] = [
                {"url": server} for server in options["servers"]
            ]
...
class PostmanToHarConverter(FormatConverter):
    ...
    def convert(self, source_path: str, target_path: Optional[str] = None, **options) -> Dict[str, Any]:
        ...
        if validate_schema:
            ...
            if format_name != "postman":
                raise ValueError(f"Expected Postman Collection file but detected {format_name}")
    ...
    def _process_postman_items(self, postman_data: Dict[str, Any], entries: List[Dict[str, Any]]) -> None:
    ...
    def _process_items(self, items: List[Dict[str, Any]], entries: List[Dict[str, Any]]) -> None:
    ...
    def _convert_request_to_entry(self, item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "response" in item and item["response"]:
            example_response = item["response"][0] if isinstance(item["response"], list) else item["response"]
            if example_response:
    ...
    def _add_request_body(self, request: Dict[str, Any], body_data: Dict[str, Any]) -> None:
        ...
        if mode == "raw":
            ...
            for param in body_data.get("formdata", []):
                ...
                params.append({
                    "name": param.get("key", ""),
                    "value": param.get("value", "") if param.get("type") == "text" else "<file>"
                })
    ...
    def _add_example_response(self, response: Dict[str, Any], example: Dict[str, Any]) -> None:
...
class PostmanToOpenApi3Converter(FormatConverter):
    ...
    def convert(self, source_path: str, target_path: Optional[str] = None, **options) -> Dict[str, Any]:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".har", mode="w") as tmp_har:
...
def convert_file(source_path: str, target_path: str, source_format: Optional[str] = None, 
                target_format: Optional[str] = None, validate_schema: bool = True, **options) -> Optional[Dict[str, Any]]:
