...
def parse_args(args: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--version",
        help="API version (for OpenAPI/Swagger output)",
        default="1.0.0"
    )
...
def main(args: Optional[List[str]] = None) -> int:
    ...
    if not source_format:
        ...
        if source_format:
            ...
            print(f"Error: Could not detect source format for '{input_path}'. Please specify with --from-format", file=sys.stderr)
            return 1
    ...
    if not target_format:
        ...
        if target_format:
            ...
            print(f"Error: Could not detect target format for '{output_path}'. Please specify with --to-format", file=sys.stderr)
            return 1
    
    # Check if conversion is available
    converter = get_converter_for_formats(source_format, target_format)
    if not converter:
        print(f"Error: No converter available for {source_format} to {target_format}", file=sys.stderr)
