...
class FileHandler:
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                if not isinstance(content, dict):
                    raise ValueError(f"Loaded content is not a dictionary: {type(content)}")
    ...
    @classmethod
    def load_and_validate(cls, file_path: Union[str, Path], schema_name: str) -> Dict[str, Any]:
        ...
        if not cls.validate(data, schema_name):
            raise ValueError(f"File {file_path} failed validation against schema '{schema_name}'")
