...
from har_oa3_converter.schemas.json_schemas import HAR_SCHEMA, OPENAPI3_SCHEMA, SWAGGER_SCHEMA, get_schema
...
class TestReadmeCliExamples:
    ...
    @pytest.mark.parametrize("command_args", [
        ...
        ["input.har", "-o", "output.yaml", "--title", "My API", "--version", "1.0.0"],
        ["input.har", "-o", "output.yaml", "--server", "https://api.example.com/v1"]
    ])
    def test_har2oa3_cli(self, command_args, sample_har_data):
        ...
        try:
            # Mock sys.argv
            with mock.patch("sys.argv", ["har2oa3"] + [arg.replace("input.har", har_path) for arg in command_args]):
                # Mock the main function to prevent actual execution
                with mock.patch("har_oa3_converter.cli.har_to_oas_cli.main") as mock_main:
    ...
    @pytest.mark.parametrize("command_args", [
        ...
        ["input.yaml", "output.json", "--from-format", "openapi3", "--to-format", "swagger"],
        ["--list-formats"]
    ])
    def test_api_convert_cli(self, command_args, sample_har_data):
        ...
        try:
            ...
            if har_path:
                command_args = [arg.replace("input.har", har_path) for arg in command_args]
...
class TestReadmeLibraryExamples:
    ...
    def test_har_to_oas3_converter(self, sample_har_data):
        ...
        try:
            ...
            try:
                ...
                with mock.patch('har_oa3_converter.utils.file_handler.FileHandler.load', return_value=sample_har_data):
                    with mock.patch('har_oa3_converter.utils.file_handler.FileHandler.save'):
    ...
    def test_har_to_oas3_converter_with_loaded_data(self, sample_har_data):
        ...
        try:
            ...
            try:
                ...
                with mock.patch('builtins.open', mock.mock_open(read_data=json.dumps(sample_har_data))):
    ...
    def test_format_converter_functions(self, sample_har_data):
        ...
        try:
            ...
            with mock.patch("har_oa3_converter.format_converter.convert_file", return_value={"openapi": "3.0.0"}):
    ...
    def test_file_handler_api(self, sample_har_data):
        ...
        try:
            ...
            with mock.patch('builtins.open', mock.mock_open(read_data=yaml.dump({"test": "data"}))):
...
class TestReadmeApiExamples:
    ...
    def test_fastapi_endpoints(self):
        ...
        assert "/formats" in router_endpoints or any("/formats" in route for route in router_endpoints)
        assert "/convert/{target_format}" in router_endpoints or any("/convert/" in route for route in router_endpoints)
