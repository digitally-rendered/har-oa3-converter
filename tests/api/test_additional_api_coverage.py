...
def test_empty_file_upload():
    ...
    try:
        ...
        assert "empty" in response.json()["detail"].lower() or "invalid" in response.json()["detail"].lower()
    finally:
        # Cleanup
...
def test_unsupported_target_format():
    ...
    try:
        ...
        with mock.patch("har_oa3_converter.api.routes.get_available_formats") as mock_formats:
...
def test_response_format_handling():
    ...
    try:
        ...
        with mock.patch("har_oa3_converter.api.routes.convert_file") as mock_convert, \
             mock.patch("har_oa3_converter.api.routes.FileHandler", autospec=True) as mock_file_handler:
            
            # Setup mock for FileHandler to return expected data
            mock_file_handler.load.return_value = {"openapi": "3.0.0", "info": {"title": "Test API"}}
