...
class TestAPIRoutes:
    ...
    def test_convert_with_invalid_format(self):
        ...
        try:
            ...
            assert response.status_code in [400, 422]  # 422 is common for FastAPI validation errors
    ...
    def test_convert_with_unsupported_content_type(self):
        ...
        try:
            ...
            assert any(error_text in response_json["detail"] for error_text in 
                      ["Failed to load file", "Invalid source file", "content type"])
        finally:
            # Clean up
            if os.path.exists(file_path):
                os.unlink(file_path)
    ...
    def test_convert_with_invalid_json(self):
        ...
        try:
            ...
            assert any(error_text in response_json["detail"] for error_text in 
                      ["Invalid JSON", "Failed to load", "Invalid source file", "JSON"])
        finally:
            # Clean up
            if os.path.exists(file_path):
                os.unlink(file_path)
    ...
    def test_convert_with_different_accept_header(self, sample_har_content):
        ...
        try:
            ...
            with open(har_file_path, "rb") as f:
                ...
                response = client.post("/api/convert/openapi3", files=files, headers=headers)
            
            # The implementation may or may not handle the Accept header as expected
            if response.status_code == 200:
                # If successful, verify YAML content type or content
                if "content-type" in response.headers and "yaml" in response.headers["content-type"]:
                ...
                assert response.status_code in [400, 406]  # 406 Not Acceptable is common for unsupported formats
                assert "detail" in response.json()
        finally:
            # Clean up
            if os.path.exists(har_file_path):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with open(har_file_path, "rb") as f:
                    ...
                    response = client.post("/api/convert/openapi3", files=files, data=form_data)
    ...
    def test_with_mocked_converter(self):
        ...
        with patch("har_oa3_converter.api.routes.convert_file") as mock_convert:
            ...
            mock_convert.return_value = {"openapi": "3.0.0", "info": {"title": "Test API"}}
