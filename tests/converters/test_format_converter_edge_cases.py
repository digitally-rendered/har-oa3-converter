...
class TestEdgeCaseSchemaConversion:
    ...
    def test_convert_schema_with_recursive_reference(
        self, sample_openapi3_with_edge_cases
    ):
        """Test conversion of schema with recursive reference."""
        converter = OpenApi3ToSwaggerConverter()

        # Get the RecursiveObject schema from the sample
        recursive_schema = sample_openapi3_with_edge_cases["components"]["schemas"][
            "RecursiveObject"
        ]
    ...
    def test_convert_schema_with_complex_allOf(self):
        ...
        schema = {
            "allOf": [
                ...
                {
                    "type": "object",
                    "properties": {"additionalProp": {"type": "string"}},
                },
                {"required": ["id", "name"]},
            ]
        }
    ...
    def test_complex_request_body_conversion(self, sample_openapi3_with_edge_cases):
        ...
        sample_openapi3_with_edge_cases["paths"]["/edge-cases"]["post"] = {
            ...
            "requestBody": {
                ...
                "content": {
                    "application/json": {
                        "schema": {"$ref": "#/components/schemas/EdgeCaseObject"}
                    },
                    "application/xml": {
                        "schema": {
                            "type": "object",
        ...
        result = converter._convert_openapi3_to_swagger2(
            sample_openapi3_with_edge_cases
        )
