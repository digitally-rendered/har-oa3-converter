...
class TestPostmanConverters:
    ...
    def test_postman_to_har_conversion(self, sample_postman_file):
        ...
        try:
            ...
            get_entry = next(entry for entry in entries if entry["request"]["method"] == "GET")
            ...
            post_entries = [entry for entry in entries if entry["request"]["method"] == "POST"]
            assert len(post_entries) >= 2
            
            # At least one POST entry should have a JSON body
            json_post = next((entry for entry in post_entries if 
                            "postData" in entry["request"] and 
                            "application/json" in entry["request"]["postData"].get("mimeType", "")), 
                            None)
            assert json_post is not None
            
            # At least one entry should have a form body
            form_post = next((entry for entry in post_entries if 
                            "postData" in entry["request"] and 
                            "application/x-www-form-urlencoded" in entry["request"]["postData"].get("mimeType", "")), 
                            None)
