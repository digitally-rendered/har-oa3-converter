...
class TestFormatRegistry:
    ...
    def test_get_converter_for_formats(self):
        ...
        converter_cls = get_converter_for_formats("invalid_format", "another_invalid_format")
    ...
    def test_guess_format_from_file(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as tmp_file:
            tmp_file.write(b'{"openapi": "3.0.0", "info": {"title": "Test", "version": "1.0.0"}, "paths": {}}')
            tmp_file.flush()
            openapi3_json_path = tmp_file.name
        
        # Test with OpenAPI 3 YAML file
        with tempfile.NamedTemporaryFile(suffix=".yaml", delete=False) as tmp_file:
            tmp_file.write(b'openapi: "3.0.0"\ninfo:\n  title: Test\n  version: 1.0.0\npaths: {}')
            tmp_file.flush()
            openapi3_yaml_path = tmp_file.name
        
        # Test with Postman collection file
        with tempfile.NamedTemporaryFile(suffix=".postman_collection.json", delete=False) as tmp_file:
            tmp_file.write(b'{"info": {"name": "Test Collection", "schema": "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"}, "item": []}')
            tmp_file.flush()
            postman_path = tmp_file.name
        
        try:
            # Test format detection
            with patch("har_oa3_converter.converters.schema_validator.detect_format") as mock_detect:
            ...
            for path in [har_path, openapi3_json_path, openapi3_yaml_path, postman_path]:
    ...
    def test_convert_file(self, mock_get_converter, mock_validate):
        ...
        with tempfile.NamedTemporaryFile(suffix=".har", delete=False) as tmp_file:
            tmp_file.write(b'{"log": {"version": "1.2", "creator": {"name": "Test", "version": "1.0"}, "entries": []}}')
            tmp_file.flush()
    ...
    @patch('har_oa3_converter.converters.format_registry.validate_file')
    @patch('har_oa3_converter.converters.format_registry.get_converter_for_formats')
    def test_convert_file_with_auto_format_detection(self, mock_get_converter, mock_validate, mock_guess):
    ...
    def test_convert_file_validation_failure(self, mock_validate):
        ...
        try:
            ...
            with pytest.raises(ValueError) as excinfo:
                convert_file(source_path, "output.json", source_format="har", target_format="openapi3")
            
            # Get the actual error message
            error_str = str(excinfo.value)
            
            # Verify the error message contains the expected text
            assert "Source file validation failed" in error_str, f"Expected 'Source file validation failed' in '{error_str}'"
    ...
    def test_convert_file_no_converter(self, mock_get_converter, mock_validate):
        ...
        try:
            ...
            error_str = str(excinfo.value)
            assert "No converter available for text to unknown_format" in error_str, f"Expected 'No converter available' message in '{error_str}'"
