...
def test_format_detection_failure():
    ...
    try:
        ...
        with mock.patch("sys.stderr"), mock.patch("sys.stdout"):
            ...
            with mock.patch(
                "har_oa3_converter.format_cli.guess_format_from_file", return_value=None
            ):
        ...
        with mock.patch("sys.stderr"), mock.patch("sys.stdout"):
            ...
            with mock.patch(
                "har_oa3_converter.format_cli.guess_format_from_file",
                side_effect=["openapi3", None],
            ):
                result = format_cli_main([input_path, output_path])
...
def test_unsupported_conversion():
    ...
    try:
        ...
        with mock.patch("sys.stderr"), mock.patch("sys.stdout"):
            ...
            with mock.patch(
                "har_oa3_converter.format_cli.get_converter_for_formats",
                return_value=None,
            ):
                # Mock guess_format_from_file to return formats
                with mock.patch(
                    "har_oa3_converter.format_cli.guess_format_from_file",
                    side_effect=["format1", "format2"],
                ):
                    result = format_cli_main([input_path, output_path])
...
def test_conversion_options():
    ...
    # Create valid files
    with tempfile.NamedTemporaryFile(
        delete=False, suffix=".har"
    ) as input_file, tempfile.NamedTemporaryFile(
        delete=False, suffix=".yaml"
    ) as output_file:
        input_file.write(
            b'{"log": {"version": "1.2", "creator": {"name": "test"}, "entries": []}}'
        )
        input_path = input_file.name
        output_path = output_file.name
    ...
    try:
        ...
        with mock.patch("sys.stdout"):
            ...
            with mock.patch(
                "har_oa3_converter.format_cli.convert_file"
            ) as mock_convert:
                ...
                assert (
                    ...
                    and kwargs["description"] == "Custom description"
                )
                assert "servers" in kwargs and kwargs["servers"] == [
                    "https://api.example.com"
                ]
