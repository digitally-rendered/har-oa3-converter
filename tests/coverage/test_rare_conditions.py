...
def test_permission_denied():
    ...
    try:
        ...
        with patch('har_oa3_converter.utils.file_handler.FileHandler.read_file', 
                  side_effect=PermissionError("Permission denied")):
            file_handler = FileHandler()
            with pytest.raises(PermissionError):
                file_handler.read_file("any_path.txt")  # The path doesn't matter since we're patching the method
    finally:
...
def test_api_schema_validation_timeout(client):
    ...
    with patch('har_oa3_converter.converters.format_converter.convert_file', side_effect=TimeoutError("Schema validation timeout")), \
         patch('har_oa3_converter.api.routes.convert_file', side_effect=TimeoutError("Schema validation timeout")):
