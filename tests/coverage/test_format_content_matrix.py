        ...
        with open(file_path, "rb") as f:
            ...
            if response.status_code == 200:
                ...
                if accept_header == "application/x-yaml":
                    ...
                    assert (
                        content_type.startswith(accept_header)
                        or (
                            accept_header in ["application/yaml", "text/yaml"]
                            and content_type
                            in ["application/yaml", "application/x-yaml", "text/yaml"]
                        )
                        or (
                ...
                try:
                    ...
                    if target_format in [
                        "openapi3",
                        "swagger",
                    ] and source_format not in ["postman", "har"]:
...
def test_invalid_content_types(
    client, sample_data, target_format, invalid_content_type
):
    """Test error handling for invalid content types."""
        ...
        with open(file_path, "rb") as f:
            ...
            if response.status_code != 200:
                ...
                assert (
                    ...
                    in FORMAT_CONTENT_TYPES[target_format]
                )
    finally:
        # Clean up the test file
        if os.path.exists(file_path):
...
@pytest.mark.parametrize(
    ...
    [(fmt, ctype) for fmt in FORMATS for ctype in FORMAT_CONTENT_TYPES[fmt]],
)
...
def test_timeout_handling(client, monkeypatch):
    ...
    try:
        ...
        with open(file_path, "rb") as f:
            ...
            ), f"Expected 408 status code, got {response.status_code}"
            error = response.json()
            assert "detail" in error
            assert (
...
def test_memory_error_handling(client, monkeypatch):
    ...
    try:
        ...
        with open(file_path, "rb") as f:
            ...
            ), f"Expected 413 status code, got {response.status_code}"
            error = response.json()
            assert "detail" in error
            assert (
