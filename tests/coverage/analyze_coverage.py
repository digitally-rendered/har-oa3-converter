...
def run_coverage_analysis():
    ...
    cov = coverage.Coverage(
        source=["har_oa3_converter"], omit=["*/__pycache__/*", "*/tests/*", "*/venv/*"]
    )
...
def recommend_tests(missing_by_module):
    ...
    if converter_modules:
        ...
        print(
            "   - Run: pytest tests/coverage/test_edge_cases.py::test_converter_edge_cases -v"
        )
    ...
    if schema_modules:
        print("\n4. Schema Modules:")
        print(
            "   - Run: pytest tests/coverage/test_internal_methods.py::test_json_schema_validation -v"
        )
