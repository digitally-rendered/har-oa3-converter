...
def test_memory_error_in_conversion(client):
    ...
    with patch("har_oa3_converter.api.routes.convert_file", side_effect=MemoryError("Test memory error")):
...
def test_timeout_error_in_conversion(client):
    ...
    with patch("har_oa3_converter.api.routes.convert_file", side_effect=TimeoutError("Test timeout error")):
        # Create a simple test file
        with tempfile.NamedTemporaryFile(suffix=".json") as tmp:
            tmp.write(b'{"test": "data"}')
            tmp.flush()
