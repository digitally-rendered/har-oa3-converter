...
docker exec har-oa3-test bash -c "chown -R appuser:appuser /app/reports 2>/dev/null || true"
...
docker exec -u appuser har-oa3-test bash -c "cd /app && \
    pylint har_oa3_converter/ tests/ --fail-under=9.0 || echo \"⚠️  Pylint check failed but continuing with tests\"
"
